...
class CelebADatasetWTrans(torch.utils.data.Dataset):
    """Custom Pytorch dataset class for CelebA dataset with transformation matrix"""

    def __init__(self, csv_file, root_dir, attr_list=None, transform=None):
        self.attributes = pd.read_csv(csv_file)
        self.root_dir = Path(root_dir)
        self.transform = transform
        self.csv_header = ['filename', '5_o_Clock_Shadow','Arched_Eyebrows','Attractive','Bags_Under_Eyes',
                           ...
                           'Wearing_Necktie','Young','a11','a12','a13','a21','a22','a23']

        if attr_list:
            self.attr_filter = []
            for attr_name in attr_list:
                assert self.csv_header.count(attr_name) > 0, f'Unknown attribute'
                self.attr_filter.append(self.csv_header.index(attr_name))
        else:
            self.attr_filter = [*(range(1, 41))] # exclude filename 
    ...
    def __getitem__(self, index: int):
        X = Image.open(self.root_dir / 'img_align_celeba' / self.attributes.iloc[index]['filename'])
        if self.transform:
            X = self.transform(X)
        
        # get attributes from csv file
        attr = self.attributes.iloc[index, self.attr_filter].values.flatten().tolist()
        xform_matrix = [[self.attributes.iloc[index]['a11'], self.attributes.iloc[index]['a12'], self.attributes.iloc[index]['a13']],
                        [self.attributes.iloc[index]['a21'], self.attributes.iloc[index]['a22'], self.attributes.iloc[index]['a23']]]
        attr = torch.tensor(attr)
        attr = torch.div(attr + 1, 2, rounding_mode="floor") # map from {-1, 1} to {0, 1}
        xform_matrix = torch.tensor(xform_matrix)
        return X, attr, xform_matrix
...
class CelebA:
    ...
    def __post_init__(self):
        ...
        if self.with_xfrom:
            train_dataset = CelebADatasetWTrans(csv_file='/tmp2/dataset/celeba_tm/celeba_tm_train.csv', 
                                                root_dir='/tmp2/dataset/celeba_tm', attr_list=self.attr_list, transform=val_transform)
            val_dataset = CelebADatasetWTrans(csv_file='/tmp2/dataset/celeba_tm/celeba_tm_val.csv', 
                                              root_dir='/tmp2/dataset/celeba_tm', attr_list=self.attr_list, transform=val_transform)
        else:
            # dataset source from pytorch official
...
class FairFaceDatasetWTrans(torch.utils.data.Dataset):
    """Custom pytorch Dataset class for FairFace dataset with transformation matrix"""
    ...
    def __getitem__(self, index: int):
        ...
        race = self.attributes.iloc[index]['race']
        # convert raw attributes to binary labels
        attr = [self.race_dict[race], self.gender_dict[gender], self.age_dict[age]]
        xform_matrix = [[self.attributes.iloc[index]['a11'], self.attributes.iloc[index]['a12'], self.attributes.iloc[index]['a13']],
                        [self.attributes.iloc[index]['a21'], self.attributes.iloc[index]['a22'], self.attributes.iloc[index]['a23']]]
        attr = torch.tensor(attr)
        xform_matrix = torch.tensor(xform_matrix)
...
class FairFace:
    ...
    def __post_init__(self):
        ...
        # select the dataset race definition
        if self.with_xfrom:
            train_dataset = FairFaceDatasetWTrans(self.train_csv, self.root, transform=val_transform)
            val_dataset = FairFaceDatasetWTrans(self.val_csv, self.root, transform=val_transform)
        else:
