        ...
        for attr_idx, attr_name in enumerate(self.attr_list):
            ...
            for idx, stats_dict in enumerate(stats_dict_list):
                ...
                line = mlines.Line2D(list(map(lambda x: 1.0-x, stats_dict[to_dict_key(self.fairness_matrix)][:epoch_axis.shape[0],attr_idx])), 
                               ...
                               marker=self.marker_list[idx], markersize=self.markersize, color=self.color_list[idx], fillstyle='none', linestyle=" ")
                axs.add_line(line)
            ...
            axs.set_ylabel('Total Accuracy (higher the better)', fontsize="16",)
            axs.legend(legend_list[1:], fontsize="16", loc="lower left")
...
def main(args):
    ...
    visualizer.draw_status_per_attribute(stats_list=stats_list, legend_list=args.legends, description=args.description)
...
def get_args():
    ...
    parser.add_argument("--description", type=str, help="words to put on the bottom of the graph")
