...
def main(args):
    ...
    match args.adv_type:
        ...
        case "eyeglasses":
            fairface = FairFace(batch_size=args.batch_size, with_xfrom=True,
    ...
    losses = Losses(loss_type=args.loss_type, fairness_criteria=args.fairness_matrix, 
                    pred_type='binary', dataset_name='FairFaceAge', soft_label=False)
        ...
        for batch_idx, bundle in enumerate(train_dataloader):
            match args.adv_type:
                ...
                case "eyeglasses":
                    ...
                    data, raw_label, theta = data.to(device), raw_label.to(device), theta.to(torch.float32).to(device)
